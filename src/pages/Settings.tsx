...
function ZApiTemplateModal({ workspaceId, editTemplate, onClose, onSaved }: {
  ...
  async function handleSave() {
    ...
    if (msgType === "button_list" && buttons.every((b) => !b.label.trim())) return;
    setSaving(true);
    try {
      const payload = {
      ...
      toast({ title: "Erro ao salvar template", description: err instanceof Error ? err.message : "Tente novamente.", variant: "destructive" });
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg rounded-2xl p-6 space-y-5 my-auto"
      ...
      >
        ...
        <div>
          ...
          <input className="input-agro w-full" placeholder="Ex: Cobrança Mensal, Boas-vindas..." value={name} onChange={(e) => setName(e.target.value)} />
        </div>
                ...
                >
                  <div className="flex items-center gap-2 mb-0.5">
                    <div className="w-3 h-3 rounded-full flex items-center justify-center shrink-0"
                      style={{ border: sel ? "none" : "2px solid rgba(139,92,246,0.4)", background: sel ? "linear-gradient(135deg, #a78bfa, #7c3aed)" : "transparent" }}
                    >
        ...
        {/* Header */}
        <div>
          <FieldLabel>Header / Título <span className="font-normal text-agro-muted-2 normal-case">(opcional)</span></FieldLabel>
          <input className="input-agro w-full" placeholder="Ex: Aviso importante, Cobrança vencida..." value={headerText} onChange={(e) => setHeaderText(e.target.value)} maxLength={60} />
          <p className="text-[10px] text-agro-muted mt-1">{headerText.length}/60 · Exibido em negrito acima do corpo</p>
        </div>
        ...
        <div>
          ...
          <p className="text-[10px] text-agro-muted mt-1">
            Use <span className="font-mono text-amber-400">{"{{1}}, {{2}}, ..."}</span> — as variáveis serão mapeadas no wizard de campanha
          </p>
        </div>
        ...
        <div>
          ...
          <input className="input-agro w-full" placeholder="Ex: Ignore se já pagou · www.empresa.com.br" value={footer} onChange={(e) => setFooter(e.target.value)} />
          <p className="text-[10px] text-agro-muted mt-1">Exibido em fonte menor abaixo do corpo</p>
        </div>

        {/* Buttons (only for button_list) */}
        {msgType === "button_list" && (
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {buttons.length < 3 && (
                <button type="button" onClick={addButton}
                  className="flex items-center gap-1 text-[11px] font-semibold text-agro-green hover:text-white transition-colors px-2 py-0.5 rounded-md hover:bg-agro-green/15"
                  style={{ border: "1px solid rgba(63,176,108,0.25)" }}
                >
            ...
            <p className="text-[10px] text-agro-muted mt-1.5">Máximo 3 botões · até 20 caracteres cada</p>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button
            ...
            style={!canSave && !saving ? { background: "rgba(139,92,246,0.2)", border: "1px solid rgba(139,92,246,0.2)" } : undefined}
          >
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0a110e" }}>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* ── Templates Z-API ──────────────────── */}
        <div className="animate-fade-up-delay-1">
          <DarkCard title="Templates Z-API" subtitle="Modelos com header, botões e footer para disparos Z-API">
              ...
              <div className="space-y-2">
                {zApiTemplates.map((tpl) => (
                  ...
                  >
                    ...
                    >
                      <LayoutTemplate className="w-3.5 h-3.5" style={{ color: "#a78bfa" }} />
                    </div>

                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-agro-text truncate">{tpl.name}</span>
                        <span className="text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0"
                          style={tpl.message_type === "button_list"
                            ? { background: "rgba(139,92,246,0.15)", color: "#a78bfa" }
                            : { background: "rgba(63,176,108,0.1)",  color: "#3fb06c" }
                          }
                        >
                          {tpl.message_type === "button_list" ? `${tpl.buttons.length} botão${tpl.buttons.length !== 1 ? "es" : ""}` : "Texto"}
                        </span>
                      </div>
                      <p className="text-xs text-agro-muted truncate mt-0.5">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        onClick={() => { setZApiTplEdit(tpl); setShowZApiTplModal(true); }}
                      ...
                      <button
                        onClick={async () => {
                          await supabase.from("z_api_templates").delete().eq("id", tpl.id);
            ...
            <button
              onClick={() => { setZApiTplEdit(null); setShowZApiTplModal(true); }}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium hover:text-white hover:bg-white/10 transition-colors"
              style={{ color: "#a78bfa", border: "1px solid rgba(139,92,246,0.3)" }}
            >
              <Plus className="w-3.5 h-3.5" />
              {zApiTemplates.length > 0 ? "Criar outro template" : "Criar primeiro template"}
            </button>
          </DarkCard>
        </div>
