...
Deno.serve(async (req: Request) => {
  ...
  if (action === "pause") {
    await supabase.from("shooting_campaigns").update({ status: "paused" }).eq("id", campaign_id);
    return new Response(JSON.stringify({ ok: true }));
  }

  if (action === "cancel") {
    await supabase.from("shooting_campaigns")
  ...
  if (action === "resume") {
    await supabase.from("shooting_campaigns").update({ status: "sending" }).eq("id", campaign_id);
    return new Response(JSON.stringify({ ok: true }));
  }
  ...
  const delayMs         = Math.ceil(60_000 / sendingSpeed);

  for (const msg of pendingMessages) {
    const { data: freshCampaign } = await supabase
      .from("shooting_campaigns").select("status").eq("id", campaign_id).single();
    ...
    const components = buildMessageComponents(
      template as Parameters<typeof buildMessageComponents>[0], mapping, row
    );
    ...
    if ("wamid" in result) {
      await supabase.from("shooting_messages")
        ...
        .eq("id", msg.id);
      await supabase.rpc("increment_campaign_counters", { p_campaign_id: campaign_id, p_counter_name: "sent_count" });
    } else {
      ...
      if (isRetryable && retryCount < (msg.max_retries ?? 3)) {
        ...
        await supabase.from("shooting_messages")
          .update({ status: "failed", failed_at: new Date().toISOString(), error_code: result.code, error_message: result.error, retry_count: retryCount })
          .eq("id", msg.id);
        await supabase.rpc("increment_campaign_counters", { p_campaign_id: campaign_id, p_counter_name: "failed_count" });
      }
    }
  ...
  const { count: pending } = await supabase
    .from("shooting_messages").select("id", { count: "exact", head: true })
